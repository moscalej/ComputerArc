...
int main(int argc, char **argv) {
	...
	bool isGlobalHist;
	if (strcmp(elemnts[3], "local_history") == 0) {
		isGlobalHist = false;
	} else if (strcmp(elemnts[3], "global_history") == 0) {
		isGlobalHist = true;
	} else {
		fprintf(stderr, "Error in input file: cannot read config\n");
		exit(5);
	...
	bool isGlobalTable;
	if (strcmp(elemnts[4], "local_tables") == 0) {
		isGlobalTable = false;
	} else if (strcmp(elemnts[4], "global_tables") == 0) {
		isGlobalTable = true;
	} else {
		fprintf(stderr, "Error in input file: cannot read config\n");
		exit(6);
	...
	bool isShare;
	if (strcmp(elemnts[5], "using_share") == 0) {
		isShare = true;
	} else if (strcmp(elemnts[5], "not_using_share") == 0) {
		isShare = false;
	} else {
		fprintf(stderr, "Error in input file: cannot read config\n");
		exit(7);
