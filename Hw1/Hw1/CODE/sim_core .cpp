...
void core::CoreClock()
{
	...
	if (split_regfile == false)
	{
		...
		{
			wb();
			_pipeStageState[WRITEBACK].cmd.opcode = CMD_NOP;
			_pipeStageState[WRITEBACK].cmd.dst = 0;
			_pipeStageState[WRITEBACK].cmd.src1 = 0;
			//_pipeStageState[WRITEBACK].cmd.src2 = 0;
			_pipeStageState[WRITEBACK].src1Val = 0;
			_pipeStageState[WRITEBACK].src2Val = 0;
	...
	if (_pipeStageState[MEMORY].cmd.dst == _pipeStageState[EXECUTE].cmd.src1 || _pipeStageState[MEMORY].cmd.dst == _pipeStageState[EXECUTE].cmd.src2)
	{
		if (_pipeStageState[MEMORY].cmd.opcode == CMD_LOAD  )
		{
			...
			_pipeStageState[DECODE].cmd = _pipeStageState[EXECUTE].cmd;
			_pipeStageState[EXECUTE].cmd.opcode = CMD_NOP;
			_pipeStageState[EXECUTE].cmd.dst = 0;
			_pipeStageState[EXECUTE].cmd.src1 = 0;
			_pipeStageState[EXECUTE].cmd.src2 = 0;
